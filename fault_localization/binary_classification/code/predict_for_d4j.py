...
if __name__ == "__main__":
    ...
    for fix_pattern in fix_patterns:
        ...
        pretrain_vectors = load_from_file(os.path.join("../data/voc_for_all/", "vectors.pkl"))
        HIDDEN_DIM = 50
        LABELS = 2
        USE_GPU = True
        ...
        model = BinaryClassifier(pretrain_vectors, EMBEDDING_DIM, MAX_TOKENS, 400, 0.15, 2, 2, 4)
        if USE_GPU:
            model.cuda()
        ...
        with torch.no_grad():
            for project in d4j_data:
                ...
                print(length)
                k = 800   #Avoiding insufficient GPU memory
                if length < k:
