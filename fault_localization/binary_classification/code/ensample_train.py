...
if __name__ == "__main__":
    ...
    pretrain_vectors = load_from_file(os.path.join(root_dir, "vectors.pkl"))
    HIDDEN_DIM = 50
    EPOCHS = 10
    BATCH_SIZE = 64
    LABELS = 2
    ...
    model = BinaryClassifier(pretrain_vectors, EMBEDDING_DIM, MAX_TOKENS, 400, 0.15, 2, 2, HEAD_NUMS)
    if USE_GPU:
        model.cuda()
    ...
    for current_pattern in pattern_list:
        if current_pattern == fix_pattern:
            continue
        root = "../data/voc_for_all/{}/".format(current_pattern)
        ...
        for epoch in range(EPOCHS):
            ...
            if val_acc_[-1] > best_val_acc:
                model_save_dir = "./model_save/weight_all/{}".format(fix_pattern)
        ...
        model.load_state_dict(torch.load("./model_save/weight_all/{}/model_params.pkl".format(fix_pattern)))
        model.eval()
    ...
    print("start fix pattern:" + fix_pattern)
    root = "../data/voc_for_all/{}/".format(fix_pattern)
