...
if __name__ == "__main__":
    ...
    output_data_dir = "../data/voc_for_all/"
    ...
    for current_pattern in pattern_list:
        ...
        all_sets[current_pattern] = token_seq_dataset
    ...
    for current_pattern in pattern_list:
        for part in all_sets[current_pattern]:
            normal_corpus = []
            normal_labels = []
            for tag in all_sets[current_pattern][part]:
                for token_seq in all_sets[current_pattern][part][tag]:
                ...
                normal_labels += [tags.index(tag)] * len(all_sets[current_pattern][part][tag])
            random.seed(shuffle_seed)
            random.shuffle(normal_corpus)
            random.seed(shuffle_seed)
            random.shuffle(normal_labels)

            output_data_dir = "../data/voc_for_all/" + current_pattern + "/"
            if not os.path.exists(output_data_dir):
                os.makedirs(output_data_dir)
