...
if __name__ == "__main__":
    ...
    model = MultiClassifier(embeddings, EMBEDDING_DIM, MAX_TOKENS, 400,0.35, 1, 11, 8)
    ...
    optimizer = torch.optim.Adam(parameters, lr=0.0005,weight_decay=1e-2)
    ...
    model.load_state_dict(torch.load(
        "./model_save_d4j/{}/model_params.pkl".format(
            part)))
    model.eval()
    fix_count = 0
    cannot_fix_list = []
    ...
    while i < len(test_x):
        ...
        earliest_appear_labels = torch.gather(test_labels, 1,
                                              torch.argmax(torch.where(test_labels == 0, neg_inf, output.data),
                                                           axis=1).unsqueeze(-1))
